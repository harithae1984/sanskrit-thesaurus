...
function generateMarkdownReview(entries: KoshaEntry[]): string {
...
function generateTSVReview(entries: KoshaEntry[]): string {
  const headers = [
    'ID',
    'Headword',
    'Headword IAST',
...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: ts-node scripts/review-entries.ts <path-to-entries-json> [output-dir]');
    console.error('Example: ts-node scripts/review-entries.ts data/entries.json');
    process.exit(1);
  }
  ...
  try {
    ...
    const entries: KoshaEntry[] = JSON.parse(entriesData);
...
export { generateMarkdownReview, generateTSVReview };
